r"""Remove a Trailing Filename Suffix.

Originally used for Traffic Control Order (TCO) file cleanup.
Selected folder only. Only the trailing suffix is removed; the same text elsewhere in the name stays unchanged. The match is case-sensitive.
Run: py .\remove_filename_suffix.py
"""
...
SUFFIX = "-NLT"  # Exact trailing text to remove before the extension.
RECURSIVE = False
EXTENSIONS = (".txt",)
# =====================================================
...
def validate_settings():
    if not SUFFIX:
        raise ValueError("Set SUFFIX to the text to remove.")
    check_filename("example" + SUFFIX)
...
def renamed_filename(filename, parent_name):
    ...
    if not stem.endswith(SUFFIX):
        return filename
    shortened = stem[:-len(SUFFIX)]
    return shortened + extension if shortened else filename
