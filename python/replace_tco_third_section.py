r"""Replace the Third TCO Section.

Originally used for Traffic Control Order (TCO) file cleanup.
Includes subfolders.
Run: py .\replace_tco_third_section.py
"""
...
REPLACEMENT = ""  # REQUIRED: choose the replacement. Old job example: "2500".
MATCH_TEXT = ()  # REQUIRED: choose matching substrings. Old example: ("0", "25").
RECURSIVE = True
EXTENSIONS = (".pdf", ".docx")
# =====================================================
...
def validate_settings():
    if not MATCH_TEXT or any(not text for text in MATCH_TEXT):
        raise ValueError("MATCH_TEXT must contain nonempty strings.")
    if not REPLACEMENT:
        raise ValueError("Set REPLACEMENT for this cleanup job; the old example was 2500.")
    check_filename(REPLACEMENT)
    if "-" in REPLACEMENT:
        raise ValueError("REPLACEMENT must be one section, without a hyphen.")


def renamed_filename(filename, parent_name):
    stem, extension = os.path.splitext(filename)
    parts = stem.split("-")
    if len(parts) > 2 and any(text in parts[2] for text in MATCH_TEXT):
        parts[2] = REPLACEMENT
