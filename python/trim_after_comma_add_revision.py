r"""Keep Text Before a Comma and Add a Revision.

Originally used for Traffic Control Order (TCO) file cleanup.
Selected folder only.
Run: py .\trim_after_comma_add_revision.py
"""
...
REVISION_SUFFIX = "-R0"  # Text to append before the extension.
RECURSIVE = False
EXTENSIONS = ()
# =====================================================
...
def validate_settings():
    if not REVISION_SUFFIX:
        raise ValueError("Set REVISION_SUFFIX to the revision to append.")
    check_filename("example" + REVISION_SUFFIX)
...
def renamed_filename(filename, parent_name):
    ...
    if "," not in stem:
        return filename
    kept = stem.split(",", 1)[0].strip()
    if not kept:
        raise ValueError("There is no filename text before the comma.")
    if not kept.endswith(REVISION_SUFFIX):
        kept += REVISION_SUFFIX
