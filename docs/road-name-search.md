...
- Road requests include the original 15 attributes plus `RevisionDate`, geometry in EPSG:4326, GeoJSON, `OBJECTID` ordering and pages of 2,000. Pagination checks both ArcGIS transfer-limit locations and advances `resultOffset` by the returned page length. A full page without a flag is followed by another request. Repeated record IDs are deduplicated; a stalled page produces an error. At 10,000 segments, the tool stops and explicitly asks for a narrower search.
...
## Segment revision dates and CSV download

The Roads layer publishes `RevisionDate` on individual centerline segments, not on unique road names. A live aggregate check on September 21, 2026 found 76,388 segments and 76,388 populated revision dates. This is an observation, not an assumed record count or completeness guarantee. A revision can reflect any record edit; the field does not identify when the road name itself changed or provide an edit history.

Each result row, details panel and popup shows the latest available revision among that group's segments, in UTC. Missing or invalid dates show **Not provided**. The name search retrieves each segment's date in the existing paginated query; no additional per-record requests are made.

**Download CSV (newest first)** exports the currently displayed search results with one row per centerline segment. `RevisionDateUTC` is an ISO 8601 UTC timestamp; the original 15 attributes follow it. Rows sort by revision descending, then by `OBJECTID` ascending for ties, with missing/invalid dates blank and last. Export sorting does not change pagination, the alphabetical grouped results or map selection. All road centerlines remain visible on the map.

The file is generated locally from the completed search, with no extra GIS download. Its UTF-8 BOM, quoted cells, doubled quotes and CRLF line endings support spreadsheet import and names containing accents, commas, quotes or newlines. Potential formula text receives a leading apostrophe so spreadsheet software treats it as text.

The download is disabled before results arrive, after Clear, for empty results and after errors. A new search clears the previous export; cancelled or stale requests cannot restore it. Searches reaching the 10,000-segment limit display a **partial CSV** button, use a `-partial-` filename and set `SearchResultsComplete=false` on every exported row. Other completed searches use `true`. Partial files contain only the loaded segments, sorted by date; they are not necessarily the newest matches across the full dataset. Narrow the search for a complete export.
...
The segment-date and CSV follow-up passed 18 focused Node tests and 27 browser checks. New coverage verifies the latest revision within a group, newest-first segment exports with deterministic ties and missing dates last, CSV parsing of Unicode/quotes/commas/line breaks, spreadsheet formula protection, live download counts and dates, disabled/stale download states, and partial-result labels in the button, filename and CSV. Desktop and mobile screenshots were inspected. JavaScript syntax and diff whitespace checks passed; unrelated test suites were not repeated.
