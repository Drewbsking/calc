...
- All road centerlines remain visible as thin gray lines beneath the community boundaries and search results, including before searching, after no matches, and after Clear. Communities have thin gray outlines and nearly transparent fill. Hover/touch tooltips show community name and type. Blue matching centerlines become thicker brown lines when selected. There is no imagery basemap or tile layer. Search-match names remain available in the results/details panel and click popups.
- At Leaflet zoom 12 and closer, the road-network image labels county primary roads, city major roads, and highways, whether or not they match the search. The label class uses the published `Act51RoadType` values `County Primary`, `City Major`, `Highway State`, `Highway Interstate` and `Highway US`, with `CartographicName` as the text. Labels follow the road, use dark text with a white halo and grow slightly at zoom 14. ArcGIS places names where space allows, so additional names can fit as the user zooms closer. Zooming out below 12 hides labels. Only the label class is filtered; all centerlines are still rendered. Labels use the existing viewport image request, without extra feature downloads or new layers.
...
The primary-road-label follow-up passed 25 browser checks plus JavaScript syntax and diff whitespace checks. New coverage verifies actual label pixels in the exported road image at zooms 12 and 14, the approved Act 51 categories, continued rendering of all centerlines, hidden labels below zoom 12, county reset and labels alongside an active search selection. The zoomed maps and selected-road screenshot were visually inspected. The unrelated full test suites were not repeated.
